...
# TypeVar bounded to subclasses of GenericMetadata
U = TypeVar("U", bound=MetadataBase[Any])

# Alias for convenience when function accept any metadata and does not need a TypeVar
...
class TextBasedMetadata(MetadataBase[str]):
    ...
    def get_libzim_value(self) -> bytes:
        return Metadata.get_encoded(self.value)
...
class TextListBasedMetadata(MetadataBase[list[str]]):
    ...
    def get_libzim_value(self) -> bytes:
        return Metadata.get_encoded(self.join_list_with.join(self.value))
...
class DateBasedMetadata(MetadataBase[datetime.date]):
    ...
    def get_libzim_value(self) -> bytes:
        return Metadata.get_encoded(self.value.strftime("%Y-%m-%d"))
    ...
    def get_reserved_names(cls) -> list[str]:
        ...
        for field in fields(cls):
            if not isinstance(field.type, type):
                continue
            if getattr(field.type, "is_required", False):
