...
class Metadata:
    ...
    Compliance with ZIM specification is done at initialisation, and conversion to
    libzim value is available with libzim_value property.
    """

    def __init__(self, name: str, value: bytes | io.BytesIO) -> None:
        if not isinstance(value, bytes | io.BytesIO):
            raise ValueError(
                f"Unexpected type passed to {self.__class__.__name__}: "
...
def __check_for_allowed_custom_metadata_name(name: str, class_name: str):
...
class CustomTextMetadata(_TextMetadata):
    ...
    def __init__(self, name: str, value: bytes | io.BytesIO | str) -> None:
        __check_for_allowed_custom_metadata_name(name, self.__class__.__name__)
...
class LanguageMetadata(_MandatoryTextMetadata):
    ...
    def __init__(self, value: bytes | io.BytesIO | str | list[str] | set[str]) -> None:
        ...
        if isinstance(value, list | set) and not all(
            ...
            bad_types = {item.__class__.__name__ for item in value} - {"str"}
            raise ValueError(
