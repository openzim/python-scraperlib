...
def test_change_image_format(
    ...
    convert_image(
        src, dst, fmt=dst_fmt, **{"colorspace": colorspace} if colorspace else {}
    )
...
def test_optimize_webp_gif_failure(
    tmp_path: pathlib.Path, webp_image: pathlib.Path, gif_image: pathlib.Path
):
