...
rules_resp = requests.get(
    os.getenv(
        "ZIMSCRAPERLIB_RULES_YAML_URL", "https://dev.kiwix.org/zimscraperlib/rules.yaml"
    ),
    timeout=REQUEST_TIMEOUT,
)
rules_resp.raise_for_status()

rules_config = yaml.safe_load(rules_resp.content.decode())
FUZZY_RULES_VERSION = rules_config["version"]
...
export const fuzzyRulesVersion = {{ FUZZY_RULES_VERSION }}
...
if not js_parent.exists():
    ...
    (js_parent / "fuzzyRules.js").write_text(
        JINJA_ENV.from_string(js_code_template).render(
            ...
            FUZZY_RULES_VERSION=FUZZY_RULES_VERSION,
        )
    )
...
if not py_parent.exists():
    ...
    (py_parent / "rules.py").absolute().write_text(
        JINJA_ENV.from_string(py_code_template).render(
            FUZZY_RULES=FUZZY_RULES, FUZZY_RULES_VERSION=FUZZY_RULES_VERSION
        )
    )
